...
voluntarios = np.concatenate((np.arange(1, 192), np.arange(193, 253), np.arange(254, 392),np.arange(393, 500), np.arange(550, 582)))
    ...
    for i in range (n_tt):
        ...
        for k in range(len(key_press)-100):
            ...
            if key_press[k+100] == 0 or key_press[k+100] == 1: 
                ...
                if rt > 200:
                    #check correct
                    if df[0,2] == stimulus_A [0] or df[0,2] == stimulus_A [1] or df[0,2] == stimulus_A [2]:
                    ...
                    if df[0,2] == stimulus_B [0] or df[0,2] == stimulus_B [1] or df[0,2] == stimulus_B [2]:
...
nomes_coluna = ['cod_suj','resp_corretas','n_corretas', 'n_erradas', 'n_antecipadas', 'n_omissas']

np.savetxt('resultados/Abstract_six.csv', np.column_stack([voluntarios, resp_corretas,n_corretas, n_erradas, n_antecipadas, n_omissas] ), header = ','.join(nomes_coluna), delimiter=',')
