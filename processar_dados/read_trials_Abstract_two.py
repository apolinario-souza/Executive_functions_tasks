...
voluntarios = np.concatenate((np.arange(1, 257), np.arange(258, 481), np.arange(482, 500), np.arange(550, 582)))
...
for suj in voluntarios:
    ...
    for i in range (n_tt):
        ...
        df=pd.read_csv('dados/abstract_shapes_two/trials/SUJ'+str(suj)+'_trial'+str(i)+'.csv', sep=',',header=0)
...
np.savetxt('resultados/Abstract_two.csv', np.column_stack([voluntarios, resp_corretas,n_corretas, n_erradas, n_antecipadas, n_omissas] ), header = ','.join(nomes_coluna), delimiter=',')
