...
for suj in voluntarios:
    ...
    for i in range (n_tt):
        ...
        df=pd.read_csv('dados/dots_congruent/trials/SUJ'+str(suj)+'_trial'+str(i)+'.csv', sep=',',header=0)
        df = np.array(df)
                ...
                if rt > 200:
                    ...
                    if df[0,2] == stimulus_A [0] or df[0,2] == stimulus_A [1]:
                        if key_press[k+100] == correct_A:
                            ...
                            if df[0,2] == condition_cong [0] or df[0,2] == condition_cong [1] or df[0,2] == condition_cong [2] or df[0,2] == condition_cong [3] :
                                index_cong.append(i)
...
nomes_coluna = ['resp_corretas','n_corretas', 'n_erradas', 'n_antecipadas', 'n_omissas','congruente', 'incogruente','n_congruente', 'n_incogruente','com_mudanca_cong', 'sem_mudanca_cong', 'com_mudanca_inc', 'sem_mudanca_inc']

np.savetxt('resultados/Dots_congruent.csv', np.column_stack([resp_corretas,n_corretas, n_erradas, n_antecipadas, n_omissas,congruente, incogruente,n_congruente, n_incogruente,com_mudanca_cong, sem_mudanca_cong, com_mudanca_inc, sem_mudanca_inc ] ), header = ','.join(nomes_coluna), delimiter=',')
