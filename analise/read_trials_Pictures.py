...
for suj in voluntarios:
    ...
    for i in range (n_tt):
        ...
        df=pd.read_csv('dados/pictures/trials/SUJ'+str(suj)+'_trial'+str(i)+'.csv', sep=',',header=0)
        df = np.array(df)
...
np.savetxt('resultados/Pictures.csv', np.column_stack([resp_corretas,n_corretas, n_erradas, n_antecipadas, n_omissas,congruente, incogruente,n_congruente, n_incogruente,com_mudanca_cong, sem_mudanca_cong, com_mudanca_inc, sem_mudanca_inc ] ), header = ','.join(nomes_coluna), delimiter=',')
