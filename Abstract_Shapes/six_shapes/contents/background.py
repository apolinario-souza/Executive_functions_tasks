import pygame
from .constants import WIDTH, HEIGHT, N_TRIALS, BLACK, PERC_SWITCHES, PRESENTATION_TIME, INTERSTIMULUS, SUJ  
import random
        ...
        if self.clicked == True and self.trial < N_TRIALS:
            ...
            if self.cont >= INTERSTIMULUS and self.cont < INTERSTIMULUS+PRESENTATION_TIME:
                ...
                if self.sequence[self.trial] == 1:
                    self.img (win,  WIDTH*.45, (HEIGHT//2)-(WIDTH*.05),WIDTH*.10, "img/left1.png") #shape left 1
                if self.sequence[self.trial] == 2:
                    self.img (win,  WIDTH*.45, (HEIGHT//2)-(WIDTH*.05),WIDTH*.10, "img/left2.png") #shape left 1
                if self.sequence[self.trial] == 3:
                    self.img (win,  WIDTH*.45, (HEIGHT//2)-(WIDTH*.05), WIDTH*.10,"img/left3.png")# shape left 1
                if self.sequence[self.trial] == 4:
                    self.img (win,  WIDTH*.45, (HEIGHT//2)-(WIDTH*.05), WIDTH*.10, "img/right1.png")# shape right1
                if self.sequence[self.trial] == 5:
                    self.img (win,  WIDTH*.45, (HEIGHT//2)-(WIDTH*.05), WIDTH*.10, "img/right2.png")# shape right2
                if self.sequence[self.trial] == 6:
                    self.img (win, WIDTH*.45, (HEIGHT//2)-(WIDTH*.05), WIDTH*.10, "img/right3.png")# shape right3                         
            ...
            if self.cont >= INTERSTIMULUS+PRESENTATION_TIME+1 and self.cont >= INTERSTIMULUS+PRESENTATION_TIME+1+50:
                ...
                np.savetxt('trials/SUJ'+str(SUJ)+'_trial'+str(self.trial)+'.csv', np.column_stack([time,self.key, self.target] ), header = ','.join(names), delimiter=',')
                self.trial +=1                         
                #Set data
                self.current_time = []
                self.key = []
